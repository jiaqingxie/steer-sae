...
n_devices = 1
llm = HookedTransformer.from_pretrained(model_name_or_path, n_devices=n_devices)


sae_1, cfg_dict, _ = SAE.from_pretrained(
    release="gemma-scope-2b-pt-res-canonical", sae_id="layer_20/width_16k/canonical", device=device
)
...
W_dec_feature = sae_1.W_dec[435].to(device)
# W_dec_feature = torch.load("/cluster/project/sachan/jiaxie/SAE_Math/mean_vec/gemma-2-2b_mean_diff.pt", map_location="cuda")
...
def get_Wdec_feature(text, model, W_dec_feature):
    ...
    with torch.no_grad():
        ...
        hidden_acts = cache["blocks.20.hook_resid_post"]
        feature_values = torch.einsum("bsd,d->bs", hidden_acts, W_dec_feature)
